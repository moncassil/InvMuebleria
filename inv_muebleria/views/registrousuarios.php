<center><img src="barra.png" width="%50" height="%50"></center>
 <div class="contenido">
 	<div class="form_top">
 		<h2> Registro <span>Usuarios</span></h2>
    </div>
    <form class="form_reg" action="">
    
    <input class="input" type="text" placeholder="USUARIO" required  autofocus>
    <input class="input" type="password"  placeholder="CONTRASEÑA" required>
    <input class="input" type="text"  placeholder="TIPO DE USUARIO" required>
    <div class="boton">
     <input class="registrar" type="submit" value="REGISTRAR">
     <input class="limpiar" type="reset" value="LIMPIAR">
     </div>
   
    </form>
 </div >
 <center>
 <table >
        <thead>
        <tr>
            <th colspan="1"><a href="registrousuarios.html">NUEVO</a></th>
            <th colspan="4"> <center>REGISTROS DE USUARIOS</center></th> 
            </tr> 
        </thead>
        <tbody>
            <tr>
                <td>USUARIOS</td>
                <td>CONTRASEÑA</td>
                <td>TIPO DE USUARIO</td>
                <td colspan="2"> <center>OPERACION A REALIZAR</center></td>
                
            </tr>
      <!--   migue elimina esto -->
            <tr>
                <td>JGARCIA</td>
                <td>sharon</td>
                <td>cliente</td>
                <td><a href="">Modficar</a></td>
                <td><a href="">Eliminar</a></td>
            </tr>
        <!-- hasta aqui solo era para hacer pruebas -->
      <tbody> 
    </table>
  </center>